...
@router.post("/run")
async def run_strategy(authorization: str = Header(None)):
    """
    ...
    try:
        ...
        if not os.path.exists(csv_file):
            return {
                "success": False,
                "message": "Historical data not found. Please download data first."
            }
        ...
        return {
            ...
            "signals_found": len(signals),
            "trades_created": len(trades_created),
            "message": f"Strategy completed! {len(signals)} signals found."
        }
...
async def run_backtest(authorization: str = Header(None)):
    """
    ...
    try:
        ...
        if not updated_trades.empty:
            ...
            if not closed_trades.empty:
                ...
                summary = {
                    "total_trades": int(len(updated_trades)),
                    "closed_trades": int(len(closed_trades)),
                    "open_trades": int(len(updated_trades[updated_trades["trade_status"] == "OPEN"])),
                    "winning_trades": int(len(winning_trades)),
                    "losing_trades": int(len(losing_trades)),
                    "win_rate": round((len(winning_trades) / len(closed_trades) * 100), 2) if len(closed_trades) > 0 else 0,
                    "net_profit": round(float(closed_trades["net_pnl"].sum()), 2) if not closed_trades.empty else 0,
                    "avg_profit": round(float(closed_trades["net_pnl"].mean()), 2) if not closed_trades.empty else 0,
                    "max_profit": round(float(closed_trades["net_pnl"].max()), 2) if not closed_trades.empty else 0,
                    "max_loss": round(float(closed_trades["net_pnl"].min()), 2) if not closed_trades.empty else 0,
                }
